...
//		    	print_r($top);die;
				...
				$roleall = array();
	    	foreach($top as $k => $v){
	    		$rowset[] = Db::table('ycl_purchase')->where('uid',$v['uid'])->where('pron',0)->order('id desc')->group('cgdh')->select();
	    		  foreach($rowset as $k => $v){

	    		  	 foreach($v as $vk=>$vv){
	    		  	 	//未接单
	    		  	 	$vv['wjd'] = Db::table('ycl_purchase')->where('cgdh',$vv['cgdh'])->where('status',1)->count();
	    		  	 	//已接单
					    $vv['yjd'] = Db::table('ycl_purchase')->where('cgdh',$vv['cgdh'])->where('status',2)->count();
						//代下单
						$vv['dai'] = Db::table('ycl_purchase')->where('cgdh',$vv['cgdh'])->where('status',0)->count();
						//根据uid 转换对应username
						$i = Db::table('ycl_user')->where('id',$vv['uid'])->find();
					    $vv['uid']   = Db::table('ycl_user')->where('id',$vv['uid'])->value('username');
					    //获取用户头像
					    $vv['image'] = $i['image'];
						//巨单
					    $vv['ju'] = Db::table('ycl_purchase')->where('cgdh',$vv['cgdh'])->where('status',4)->count();
	    		  	 	if(!in_array($vv,$roleall)){
	    		  	 		$roleall[] = $vv;
	    		  	 	}
//						if($roleall) {
//	    		  	 	
//	    		  	 		foreach($roleall as $roleall_k => $roleall_v){
//	    		  	 			
//					    		//未接单数量
//					    		$roleall[$roleall_k]['wjd'] = Db::table('ycl_purchase')->where('cgdh',$roleall_v['cgdh'])->where('status',1)->count();
//					    		//已接单
//					    		$roleall[$roleall_k]['yjd'] = Db::table('ycl_purchase')->where('cgdh',$roleall_v['cgdh'])->where('status',2)->count();
//								//代下单
//								$roleall[$roleall_k]['dai'] = Db::table('ycl_purchase')->where('cgdh',$roleall_v['cgdh'])->where('status',0)->count();
//								
////								$roleall[$roleall_k]['uid'] = Db::table('ycl_user')->where('id',$roleall_v['uid'])->value('username');//根据uid 转换对应username
//								//巨单
//								$roleall[$roleall_k]['ju'] = Db::table('ycl_purchase')->where('cgdh',$roleall_v['cgdh'])->where('status',4)->count();
//			//					我淘的菜//数量
////								print_r($dai);die;
//					   			}
//							}
...
//  添加采购单页面
     public function neworder(Request $request) {
         ...
         $letter = preg_match("/^[a-zA-Z\s]+$/",$shous);
...
//   	$money = cookie('goodsmoney',$aa,3600);
    	...
    	$rowset = Db::table('ycl_goods')->where('goodsname',$name)->where('city',session('city'))->select();
    	$jy = array();
...
//             print_r($rowset1);
     	...
     	if($shous !== null) {
     	    if ($letter){
                $rowset1= Db::table('ycl_goods')->wherePinyin('like', "%$shous%")->where('city',session('city'))->select();
...
//		 	$this->assign('arr',$arr);
			$rowset1 = Db::table('ycl_goods')->whereGoodsname('like', "%$shous%")->where('city',session('city'))->select();
			$abr = array();
				foreach($rowset1 as $key=>$val){
//
				if(!in_array($rowset1[$key]['goodsname'],$abr)){
				$abr[] = $rowset1[$key]['goodsname'];
//                  if($rowset1[$key]['goodsname'] == session('goods') && $rowset1[$key]['shopid'] == session('shop_id')){
//					unset($rowset1[$key]);
//				}
				}else{
				unset($rowset1[$key]);
...
//   		$arr = cookie('arr');
     		$rowset1 = Db::table('ycl_goods')->where('city',session('city'))->select();
            $abr = array();
            foreach($rowset1 as $key=>$val){
//
                if(!in_array($rowset1[$key]['goodsname'],$abr)){
...
//        print_r($shous);
    ...
    public  function neworder2(){
        $shous = input('post.val');
        $letter = preg_match("/^[a-zA-Z\s]+$/",$shous);
//        return json_encode($shous);
//        $a = session('city');
//        $where = "city = '{$a}'";
//        $where .= $shous!='' ? " AND goodsname LIKE '%{$shous}%'" : '';
//        $rowset1 = Db::table('ycl_goods')->field('goodsname')->where($where)->select();
        if($shous !== null) {
            if ($letter){
                $rowset1= Db::table('ycl_goods')->wherePinyin('like', "%$shous%")->where('city',session('city'))->select();
...
//		print_r($shopid);die;
        ...
        $arrsession = array(
                ...
                'goods' => $request->post('name1'),
                'number' => $request->post('num1'),
                'content' => $request->post('sm1'),
                'dw' => $request->post('dw'),
//              'goodsmoney' => $request->post('dj1'),
//              'money' => $request->post('xj1'),
...
//        print_r($arrsession);die;
          session('sessionsave',$arrsession);
        //print_r($aaa);die;
//		session('goods', $name1);
...
//		session('time', $time);
            ...
            $arr = array(
                'uid' 		=> session('userid'),
                'goodsname' => session('sessionsave.goods'),
                'number' => session('sessionsave.number'),
                'content' => session('sessionsave.content'),
                'danwei' => session('sessionsave.dw'),
//              'goodsmoney' => session('sessionsave.goodsmoney'),
//              'money' => session('sessionsave.money'),
//              'shop_id' => session('sessionsave.shop_id'),
//              'shop' => session('sessionsave.shop'),
//              'shop' =>session('sessionsave.shop'),
                'add_time' => session('sessionsave.time')
            );
...
//  店铺
    public function dishes() {
        ...
        foreach ($arr as $arr_k => $arr_v){
            foreach ($jy as $jy_k =>$jy_v) {
                if (in_array($arr[$arr_k]['goodsname'],$jy_v) && in_array($arr[$arr_k]['shop_id'],$jy_v)){
...
//  	cookie('jy',null); //清空cookie
     ...
     public function cpdel() {
    	...
    	unset($arr[$souy]);
//  	cookie($souy,null);
		session('sessionsave.goods', null);
		session('sessionsave.shop_id', null);
		session('sessionsave.number', null);
		session('sessionsave.content', null);
		session('sessionsave.goodsmoney', null);
		session('sessionsave.money', null);
		session('sessionsave.shop', null);
		session('sessionsavetime', null);
    	$xinarray = array_values($arr);
    	for($i=0; $i<count($xinarray); $i++) {
    		$xinarray[$i]['suoy']= $i;
    	}
    	cookie('xinarray',$xinarray);
//  	print_r(session('userid'));die;
...
//    	  	print_r($rowset);die;
     ...
     public function gmdishes() {
    	...
    	$rowset = Db::table('ycl_goods')->where('city',session('city'))->where('goodsname',$goodsname)->select();
    	$jy = array();
    	for($i = 0; $i<count($rowset);$i++){
			$book = Db::table('ycl_shop_info')->where('city',session('city'))->where('id',$rowset[$i]['shopid'])->find();
			$norm = Db::table('ycl_spec')->where('id',$rowset[$i]['goodsgg'])->find();
//			print_r($norm);die;
...
//  print_r($purchase);die;
    	...
    	if ($query) {
			$this->success('下单成功','/index/purchase/order');
		}
    }
...
//  	print_r($rowset);die;
     ...
     * 删除采购单
     */
	public function delete_cgd(){
	    $cgdh = $_POST['cgdh'];
//		print_r($cgdh);die;
	    $rs = Db::name("purchase")->where('cgdh',$cgdh)->delete();
	    if($rs){
	        return '删除成功';
        }
     ...
     * 发送采购单至群组
     */
    public function send_cgd(){
        $cgdh = $_POST['cgdh'];
        $userid = session('userid');
        $user = Db::name('user')->where('id',$userid)->find();
        $rs = Db::name("purchase")->where('cgdh',$cgdh)->find();
        $data = [
            'userid' => $user['id'],
            'status' => 2,
            'group'  => $user['work_id'],
        ...
        $row = Db::name('chat_group')->insert($data);
        if($row){
            return '已发送';
        }else{
            return '发送失败';
