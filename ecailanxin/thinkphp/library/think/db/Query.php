...
class Query
{
    ...
    public function paginate($listRows = null, $simple = false, $config = [])
    {
        ...
        if (!isset($total) && !$simple) {
            ...
            unset($this->options['order1'], $this->options['limit'], $this->options['page'], $this->options['field']);
    ...
    /**
     * 指定排序 order1('id','desc') 或者 order1(['id'=>'desc','create_time'=>'desc'])
     * @access public
    ...
    public function order($field, $order = null)
    {
        ...
        if (!isset($this->options['order1'])) {
            $this->options['order1'] = [];
        }

        if (is_array($field)) {
            $this->options['order1'] = array_merge($this->options['order1'], $field);
        } else {
            $this->options['order1'][] = $field;
        }
    ...
    {
        $this->options['order1'][] = $this->raw($field);
    ...
    /**
     * 指定Field排序 order1('id',[1,2,3],'desc')
     * @access public
     * @param  string|array $field 排序字段
    ...
    public function orderField($field, array $values, $order = '')
    {
        if (!empty($values)) {
            ...
            $this->options['order1'][$field] = $values;
        }
    ...
    public function chunk($count, $callback, $column = null, $order = 'asc')
    {
        ...
        if (isset($options['order1'])) {
            if (Container::get('app')->isDebug()) {
                throw new DbException('chunk not support call order1');
            }
            unset($options['order1']);
        }
    ...
    protected function parseView(&$options)
    {
        ...
        if (isset($options['order1'])) {
            // 视图查询排序处理
            if (is_string($options['order1'])) {
                $options['order1'] = explode(',', $options['order1']);
            }
            foreach ($options['order1'] as $key => $val) {
                if (is_numeric($key)) {
                    if (strpos($val, ' ')) {
                        list($field, $sort) = explode(' ', $val);
                        if (array_key_exists($field, $options['map'])) {
                            $options['order1'][$options['map'][$field]] = $sort;
                            unset($options['order1'][$key]);
                        }
                    } elseif (array_key_exists($val, $options['map'])) {
                        $options['order1'][$options['map'][$val]] = 'asc';
                        unset($options['order1'][$key]);
                    }
                } elseif (array_key_exists($key, $options['map'])) {
                    $options['order1'][$options['map'][$key]] = $val;
                    unset($options['order1'][$key]);
                }
            }
        }
